...
class SpinnerPicker {
    ...
    updateView() {
        ...
        for(var i=-4; i<=4; i++) {
            ...
            if(value != null) {
                this.drawText(String(value.author) + " " + String(value.track_name), [horizontalCenter, y], this.fontColor, hBoxSize + "px " + this.font, 0, Math.abs(y - (3.55 * hBoxSize)) / (4 * hBoxSize));
            }
        }
        if(this.currentAnimationStep != this.animationSteps) {
